TEXTS = {
    "uz": {
        "welcome": "🎓 *Budget Vizaga xush kelibsiz!*\n\nBu yerda chet elga ketish uchun barcha kerakli ma'lumotlar va statistikalar mavjud — barchasi sof o'zbek tilida tayyorlangan.\n\n🏠 Uyingizdan turib, to chet el universiteti yoki ishxonasi eshigigacha bo'lgan yo'lni biz sizga to'liq o'rgatamiz.\n\n✨ *Biz bilan siz quyidagilarni qila olasiz:*\n\n✅ 100 lab universitetlarga yoki ish vakansiyalariga *tekinga* hujjat topshirish\n🏆 100% va full ride grantlarni yutish sirlarini o'rganish, bir so'm ham harajat qilmasdan davlat va halqaro grantlar sohibi bo'lish\n⚡️ Hammasini atigi 1 kun ichida o'zlashtirib, hujjat topshirishni boshlash\n🌍 Istalgan davlatga, istalgan miqdorda ariza yuborish imkoniyati\n\n💡 *SOLISHTIRMA:*\n\nKonsalting firmalar narxi o'rtacha 500$-1200$ (80% foizi sifatsiz servis ko'rsatadi, atigi 1-2 ta ish vakansiyasiga/universitetiga hujjat topshirib beradi, ko'pchiligi aldab ketadi)\n\nBiz bilan esa atigi 44$ evaziga 100-200 lab universitetlarga yoki ish vakansiyalariga hujjat topshirishni 1 kunda o'rganib olasiz! O'zingiz application jarayonini nazorat qilib turasiz",
        "main_menu": "Kerakli bo'limni tanlang:",
        
        # Course viewing
        "choose_section": "Bo'limni tanlang:",
        "choose_level": "Darajani tanlang:",
        "choose_country": "🌍 Davlatni tanlang:",
        "demo_content": "📚 *Demo kontent*\n\nBu kursning qisqacha ko'rinishi. To'liq kursga kirish uchun sotib oling!",
        "buy_course": "💳 Kursni sotib olish - ${price}",
        "course_locked": "🔒 Bu kontent faqat kurs sotib olganlar uchun.\n\n💳 Kursni sotib oling: *${price}*",
        
        # Payment
        "payment_course": "💳 *Kurs to'lovi*\n\n📚 Kurs: {course_name}\n💰 Narx: ${price}\n⏱ Amal qilish muddati: 1 yil\n\n💳 *Karta:* `{card}`\n\n📲 *To'lov:*\n{methods}\n\n✅ To'lovni bajaring va *screenshot* yuboring.",
        "payment_combo": "🎓 *Universitet+Viza combo*\n\n📍 {country} — {level_name}\n💰 Narx: ${price}\n⏱ Amal qilish muddati: 1 yil\n✨ Universitetga va Vizaga topshirish kurslari ochiladi\n🎁 2 marta TEKIN konsultatsiya\n\n💳 *Karta:* `{card}`\n\n📲 *To'lov:*\n{methods}\n\n✅ To'lovni bajaring va *screenshot* yuboring.",
        "payment_consult": "📞 *Konsultatsiya to'lovi*\n\n💰 Narx: ${price}\n⏱ Davomiyligi: 30 daqiqa\n\n💳 *Karta:* `{card}`\n\n📲 *To'lov:*\n{methods}\n\n✅ To'lovni bajaring va *screenshot* yuboring.",
        "payment_consult_free": "🎁 *Sizda TEKIN konsultatsiya mavjud!*\n\nKonsultatsiya kunini tanlang:",
        "payment_received": "✅ To'lov qabul qilindi!\n\n⏳ 15 daqiqa ichida tasdiqlanadi. Iltimos kuting...",
        
        # Payment approved
        "course_approved": "✅ *To'lovingiz tasdiqlandi!*\n\n🎉 Tabriklaymiz! Kurs sizga ochildi.\n📚 Kurs: {course_name}\n⏱ Amal qilish: 1 yil\n\n🚀 Chet elda o'qishingizga omad tilaymiz!\n\n👥 Guruh:",
        "combo_approved": "✅ *To'lovingiz tasdiqlandi!*\n\n🎉 *Universitet+Viza combo faollashtirildi!*\n\n✨ Universitetga va Vizaga topshirish kurslari ochildi\n🎁 2 marta TEKIN konsultatsiya\n⏱ Amal qilish: 1 yil\n\n🚀 Muvaffaqiyatlar!\n\n👥 Guruh:",
        "consult_approved": "✅ *Konsultatsiyangiz tasdiqlandi!*\n\n📅 Sana: {date}\n⏰ Vaqt: {slot}\n\nKonsultatsiya vaqtida tayyor bo'ling.\n📱 Aloqa: @budgetvizaadmin",
        "payment_rejected": "❌ To'lovingiz tasdiqlanmadi.\n\nAdmin bilan bog'laning: @budgetvizaadmin",
        
        # Consultation
        "choose_date": "📅 Konsultatsiya kunini tanlang:",
        "enter_name": "👤 Ismingizni yozing:",
        "send_phone": "📱 Telefon raqamingizni yuboring:",
        "choose_time": "⏰ Konsultatsiya vaqtini tanlang:",
        "reminder": "⏰ *Eslatma!*\n\n30 daqiqadan keyin konsultatsiyangiz boshlanadi.\n📅 Sana: {date}\n⏰ Vaqt: {slot}\n\nTayyor bo'ling! 🎓",
        
        # Buttons
        "btn_university": "🏛️ O'qishga topshirish",
        "btn_visa": "🪪 Vizaga topshirish",
        "btn_work": "👨‍🔧 Ishga topshirish",
        "btn_combo": "🎓 Universitet+Viza",
        "btn_consult": "📊 Konsultatsiya",
        "btn_my_courses": "🗂️ Mening kurslarim",
        "btn_deadlines": "📅 Deadlaynlar",
        "btn_about": "☎️ Yordam/Aloqa",
        "btn_profile": "👤 Mening profilim",
        "btn_admin": "👤 Bog'lanish",
        "btn_bot_panel": "⚙️ Bot boshqaruvi",
        "btn_lang": "🌐 Til / Language",
        "btn_phone": "📱 Telefon yuborish",
        "btn_buy_full": "💳 To'liq kursni sotib olish",
        "back": "🔙 Orqaga",
        "main": "🏠 Asosiy",
        
        # Other
        "about": "🎓 *Budget Viza*\n\nChet elda o'qish va ishlash bo'yicha professional maslahat xizmati.\n\n🎓 Universitet+Viza combo: $66/yil\n📚 Kurs: $44/yil\n📞 Konsultatsiya: $15 (combo uchun 2 marta TEKIN)\n\n📱 Admin: @budgetvizaadmin",
        "admin_contact": "👨‍💻 *Admin:*\n\n@budgetvizaadmin",
        "invalid_input": "❌ Iltimos, ro'yxatdan tanlang.",
        "video_coming": "🎬 Tez orada video qo'shiladi.",
        "my_courses_list": "📚 *Sizning kurslaringiz:*\n\n{courses}\n\n⏱ Barcha kurslar 1 yil amal qiladi.",
        "my_courses_empty": "📚 Sizda hali kurslar yo'q.\n\n💳 Kurs sotib oling yoki 🎓 Universitet+Viza combo oling!",
        "combo_unavailable": "❌ Bu davlat va daraja uchun Universitet+Viza combo hali mavjud emas.\n\nUniversitet va Viza kurslarining ikkisi ham qo'shilgandan keyin combo ochiladi.",

        # Profile / Referral
        "profile_menu": "👤 *Mening profilim*\n\n🔗 Sizning referal linkingiz:\n`{ref_link}`\n\n👥 Referal orqali start bosganlar: {starts} ta\n💳 Referal orqali kurs sotib olganlar: {purchases} ta\n\n💰 Balansingiz: {balance:,} so'm\n\nDo'stlaringizga ushbu linkni yuboring — ular botga start bosganda va kurs sotib olganda sizga pul tushadi!",
        "btn_withdraw": "💸 Pulni yechish",
        "btn_add_card": "💳 Karta qo'shish",
        "btn_change_card": "💳 Kartani o'zgartirish",
        "ask_card_number": "💳 Karta raqamingizni kiriting (16 ta raqam):\n\nMasalan: 8600123412341234",
        "ask_card_holder": "👤 Karta egasining ism-familiyasini kiriting:\n\nMasalan: ABROR MAMATQULOV",
        "card_saved": "✅ Karta ma'lumotlari saqlandi!\n\n💳 {card_number}\n👤 {card_holder}",
        "invalid_card_number": "❌ Noto'g'ri karta raqami. 16 ta raqamdan iborat bo'lishi kerak.",
        "withdraw_no_card": "❌ Avval kartangizni qo'shing.",
        "withdraw_not_enough_starts": "❌ Yechib olish uchun kamida {min_starts} ta referal start kerak.\n\nSizda hozir: {starts} ta",
        "withdraw_no_balance": "❌ Balansingizda yechib olish uchun mablag' yo'q.",
        "withdraw_success": "✅ So'rovingiz qabul qilindi!\n\n💰 Miqdor: {amount:,} so'm\n💳 Karta: {card_number}\n\n⏳ 5 kun ichida kartangizga o'tkaziladi.",
    },
        "en": {
                    "welcome": "🎓 *Welcome to Budget Viza!*\n\nProfessional consulting for studying and working abroad!",
        "main_menu": "Choose a section:",
        
        # Course viewing
        "choose_section": "Choose section:",
        "choose_level": "Choose level:",
        "choose_country": "🌍 Choose country:",
        "demo_content": "📚 *Demo content*\n\nThis is a preview. Buy full course for complete access!",
        "buy_course": "💳 Buy course - ${price}",
        "course_locked": "🔒 This content is for course buyers only.\n\n💳 Buy course: *${price}*",
        
        # Payment
        "payment_course": "💳 *Course payment*\n\n📚 Course: {course_name}\n💰 Price: ${price}\n⏱ Valid: 1 year\n\n💳 *Card:* `{card}`\n\n📲 *Payment:*\n{methods}\n\n✅ Make payment and send *screenshot*.",
        "payment_combo": "🎓 *Universitet+Viza combo*\n\n📍 {country} — {level_name}\n💰 Price: ${price}\n⏱ Duration: 1 year\n✨ University and Visa courses unlocked\n🎁 2 FREE consultations\n\n💳 *Card:* `{card}`\n\n📲 *Payment:*\n{methods}\n\n✅ Make payment and send *screenshot*.",
        "payment_consult": "📞 *Consultation payment*\n\n💰 Price: ${price}\n⏱ Duration: 30 min\n\n💳 *Card:* `{card}`\n\n📲 *Payment:*\n{methods}\n\n✅ Make payment and send *screenshot*.",
        "payment_consult_free": "🎁 *You have a FREE consultation!*\n\nChoose date:",
        "payment_received": "✅ Payment received!\n\n⏳ Will be confirmed in 15 minutes. Please wait...",
        
        # Payment approved
        "course_approved": "✅ *Payment confirmed!*\n\n🎉 Congratulations! Course unlocked.\n📚 Course: {course_name}\n⏱ Valid: 1 year\n\n🚀 Good luck studying abroad!\n\n👥 Group:",
        "combo_approved": "✅ *Payment confirmed!*\n\n🎉 *Universitet+Viza combo activated!*\n\n✨ University and Visa courses unlocked\n🎁 2 FREE consultations\n⏱ Valid: 1 year\n\n🚀 Good luck!\n\n👥 Group:",
        "consult_approved": "✅ *Consultation confirmed!*\n\n📅 Date: {date}\n⏰ Time: {slot}\n\nBe ready at scheduled time.\n📱 Contact: @budgetvizaadmin",
        "payment_rejected": "❌ Payment not confirmed.\n\nContact admin: @budgetvizaadmin",
        
        # Consultation
        "choose_date": "📅 Choose consultation date:",
        "enter_name": "👤 Enter your name:",
        "send_phone": "📱 Share your phone number:",
        "choose_time": "⏰ Choose time:",
        "reminder": "⏰ *Reminder!*\n\nConsultation starts in 30 minutes.\n📅 Date: {date}\n⏰ Time: {slot}\n\nGet ready! 🎓",
        
        # Buttons
        "btn_university": "🏛️ Apply to University",
        "btn_visa": "🪪 Apply for Visa",
        "btn_work": "👨‍🔧 Work",
        "btn_combo": "🎓 Universitet+Viza",
        "btn_consult": "📊 Consultation",
        "btn_my_courses": "🗂️ My Courses",
        "btn_deadlines": "📅 Deadlines",
        "btn_about": "☎️ Help/Contact",
        "btn_profile": "👤 My Profile",
        "btn_admin": "👤 Contact",
        "btn_bot_panel": "⚙️ Bot Panel",
        "btn_lang": "🌐 Til / Language",
        "btn_phone": "📱 Send phone",
        "btn_buy_full": "💳 Buy full course",
        "back": "🔙 Back",
        "main": "🏠 Main",
        
        # Other
        "about": "🎓 *Budget Viza*\n\nProfessional consulting for studying and working abroad.\n\n🎓 Universitet+Viza combo: $66/year\n📚 Course: $44/year\n📞 Consultation: $15 (2x FREE with combo)\n\n📱 Admin: @budgetvizaadmin",
        "admin_contact": "👨‍💻 *Admin:*\n\n@budgetvizaadmin",
        "invalid_input": "❌ Please choose from list.",
        "video_coming": "🎬 Video coming soon.",
        "my_courses_list": "📚 *Your courses:*\n\n{courses}\n\n⏱ All courses valid for 1 year.",
        "my_courses_empty": "📚 You don't have any courses yet.\n\n💳 Buy a course or 🎓 Get Universitet+Viza combo!",
        "combo_unavailable": "❌ Universitet+Viza combo is not available for this country/level yet.\n\nIt unlocks once both University and Visa courses are added.",

        # Profile / Referral
        "profile_menu": "👤 *My Profile*\n\n🔗 Your referral link:\n`{ref_link}`\n\n👥 Referred starts: {starts}\n💳 Referred purchases: {purchases}\n\n💰 Balance: {balance:,} som",
        "btn_withdraw": "💸 Withdraw",
        "btn_add_card": "💳 Add card",
        "btn_change_card": "💳 Change card",
        "ask_card_number": "💳 Enter your card number (16 digits):",
        "ask_card_holder": "👤 Enter cardholder name:",
        "card_saved": "✅ Card saved!\n\n💳 {card_number}\n👤 {card_holder}",
        "invalid_card_number": "❌ Invalid card number. Must be 16 digits.",
        "withdraw_no_card": "❌ Please add your card first.",
        "withdraw_not_enough_starts": "❌ You need at least {min_starts} referral starts to withdraw.\n\nYou currently have: {starts}",
        "withdraw_no_balance": "❌ No balance to withdraw.",
        "withdraw_success": "✅ Request received!\n\n💰 Amount: {amount:,} som\n💳 Card: {card_number}\n\n⏳ Will be transferred within 5 days.",
    }
}



_DATA_DIR_TX = "/data" if __import__("os").path.isdir("/data") else "."
WELCOME_FILE = _DATA_DIR_TX + "/welcome_text.json"

def get_custom_welcome(lang="uz"):
    """Returns dict {text, photo, video} or None."""
    import json, os
    try:
        if os.path.exists(WELCOME_FILE):
            with open(WELCOME_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                raw = data.get(lang) or data.get("uz")
                if raw is None:
                    return None
                if isinstance(raw, str):
                    return {"text": raw, "photo": None, "video": None}
                return {"text": raw.get("text"), "photo": raw.get("photo"), "video": raw.get("video")}
    except:
        pass
    return None

def save_custom_welcome(text=None, lang="uz", photo=None, video=None):
    """Save welcome content. Pass only the fields you want to update."""
    import json, os
    try:
        data = {}
        if os.path.exists(WELCOME_FILE):
            with open(WELCOME_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
        existing = data.get(lang)
        if isinstance(existing, str):
            existing = {"text": existing, "photo": None, "video": None}
        elif not isinstance(existing, dict):
            existing = {"text": None, "photo": None, "video": None}
        if text is not None:
            existing["text"] = text
        if photo is not None:
            existing["photo"] = photo
        if video is not None:
            existing["video"] = video
        data[lang] = existing
        os.makedirs(os.path.dirname(WELCOME_FILE), exist_ok=True)
        with open(WELCOME_FILE, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False)
        return True
    except:
        return False

def t(user_id, key, **kwargs):
    from data import get_lang
    lang = get_lang(user_id)
    if key == "welcome":
        custom = get_custom_welcome(lang)
        if custom and custom.get("text"):
            return custom["text"]
    text = TEXTS.get(lang, TEXTS["uz"]).get(key, "")
    if kwargs:
        text = text.format(**kwargs)
    return text


def get_welcome_media(user_id):
    """Returns (photo_file_id, video_file_id) for the custom welcome message, or (None, None)."""
    from data import get_lang
    lang = get_lang(user_id)
    custom = get_custom_welcome(lang)
    if custom:
        return custom.get("photo"), custom.get("video")
    return None, None
